alter type record_type add value if not exists 'contract_assignment';
alter type record_type add value if not exists 'training';
alter type record_type add value if not exists 'skill';
alter type record_type add value if not exists 'performance_review';
alter type record_type add value if not exists 'continuing_education';
...
begin
  ...
  if credential_type not in (
    ...
    raise exception 'Credential type must be license, certification, education, training, continuing_education, health_clearance, or custom';
  end if;
