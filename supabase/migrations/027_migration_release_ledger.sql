...
begin
  ...
  values (
    nullif(trim(migration_path), ''),
    nullif(trim(coalesce(commit_sha, '')), ''),
    nullif(trim(coalesce(workflow_run_id, '')), ''),
    nullif(trim(coalesce(applied_by, '')), ''),
    coalesce(nullif(trim(status), ''), 'applied'),
