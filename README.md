...
After first server setup, the manual **Deploy TrustGraph to VPS** GitHub Actions workflow can update `/opt/trustgraph`. It refuses the existing VFIX host at `5.75.224.110`, runs the same guarded `tools/update-vps-from-github.sh` path as the server shell command, and checks `trustgraph-release.json` so the VPS must prove the GitHub source and current `premium_workspace_responsive_guard` bundle marker it saved. The main Pages workflow now stamps the exported release asset with `${{ github.sha }}` before upload and rejects placeholder commits, so a missing, stale, or app-shell-fallback release stamp cannot pass as a current server save.
...
The VPS updater now writes the same `server_save_contract` used by the GitHub Pages release asset, verifies that contract through the public `trustgraph-release.json`, and smokes the protected VFIX login route after the TrustGraph refresh. A server update is not accepted if the release stamp is stale, missing the current bundle marker, missing the contract, returning the app shell, or if VFIX stops responding.

To check whether the VPS has caught up to the latest GitHub Pages bundle without changing the server, run `npm run report:vps-status`. It compares the GitHub Pages and VPS `trustgraph-release.json` files, reports `vps_current` only when the commit and `premium_workspace_responsive_guard` marker match, and prints the guarded manual update command for `/opt/trustgraph` when the server still needs syncing. If the VPS release URL serves the app shell instead of JSON, the report also prints the `tools/trustgraph-nginx.conf` repair source, `/opt/fixflow-nginx/conf.d/trustgraph.conf` target, nginx test/reload commands, and VFIX boundary.
...
That handoff can now persist a hosted completion receipt in `registration_completion_receipts`. The RPC rejects localhost redirects, ties the receipt to the current user's registration intent, records the landed dashboard, and only accepts first-database-write verification after the matching `passport_initialized` or `workspace_created` intent status exists.
...
Corporate Verify now starts with a single Corporate Verify hub. It replaces stacked database command panels in the primary view with one action path for request access, review scoped rows, record review proof, export metadata-only packets, live RBAC status, open gaps, and the no-open-user-database boundary.

The Corporate Verify hub now includes a five-step access sequence for corporate role, request by email, professional approval, scoped rows, review attestation, and metadata-only export. Duplicate reviewer workflow strips stay hidden from the primary portal path so reviewers start from one hub.
...
Admin Audit can now persist the filtered export decision in `admin_audit_export_receipts`. The receipt records the active filters, recommended export, audit row counts, verification-case/data-rights/release context, and the metadata-only boundary so Admin exports are backed by live Supabase proof rather than browser-only downloads.
...
The signed-in database path now also includes a Live rows checklist inside the real database mission control. It gives the operator a seven-step flow for hosted login, live Supabase rows, proof reload, Corporate Verify review, completion receipt, working-data export, and VPS release JSON before accepting any preview data.
...
- `063_corporate_visible_passport_rows.sql`: Corporate Verify scoped-row RPC that returns only active-RBAC, approved, non-expired, consent-scoped Passport rows and excludes raw private files or preview data.
- `064_registration_completion_receipts.sql`: hosted registration completion receipts that prove email/auth redirect landed on the hosted dashboard, not localhost, and persist selected portal, first database write, intent status, and no-preview-data boundary.
- `065_admin_audit_export_receipts.sql`: Admin audit export receipts that persist filtered scope, recommended export, audit row counts, verification-case/data-rights/release context, metadata-only boundary, and no-preview-data proof.
...
The public login and registration card keeps account recovery visible in the main form: resend verification, reset password, and copied hosted-link repair stay beside the credentials while the heavier proof receipts remain hidden from the first-screen buyer/user path. The visible route now starts from one login portal desk for Professional registration, Professional login, Corporate registration, and Corporate login, followed by the credential command and form; older route, switchboard, acceptance, and pricing proof boards stay preserved for export coverage without stacking above credentials.
...
The corporate portal studio final pass tightens Corporate Verify and Company Admin into one daily workflow. Corporate reviewers now see the database access compass, request form, approval list, filters, review queue, and scoped user rows before older proof receipts; Company Admin keeps unlock status, setup command, setup tabs, active setup form, and Corporate Verify handoff first. Duplicate proof panels stay in the bundle for exports and CI markers, but they are collapsed from the working UI so controls remain clickable and mobile-safe.
