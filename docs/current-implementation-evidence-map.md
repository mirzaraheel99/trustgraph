...
Live Supabase migrations currently run through `046_registration_intent_professional_status.sql`. This includes `043_account_context_rpc.sql` for signed-in profile, organization membership, and active role loading after hosted login; `044_registration_intents.sql` for public portal intent rows; `045_registration_intent_status.sql` for Corporate `workspace_created` completion; and `046_registration_intent_professional_status.sql` for Professional `passport_initialized` completion. The RLS guard verifies protected-table enablement across 23 tables before every hosted deployment, and the app exposes visible account-context, registration intent, and RLS repair markers so operators can distinguish code readiness from a Supabase project that still needs the organization recursion repair, account-context RPC, or registration intent completion RPCs applied.
