...
| Corporate account | `src/accountRepository.ts`, Account panel, Corporate provisioning packet, corporate setup triage board, team panels, member roster proof, account-context RPC proof | `supabase/migrations/009`, `020`, `021`, `022`, `043`, smoke corporate checks |
...
- Corporate database unlock path: top Corporate Verify bridge from company workspace and reviewer RBAC to access request, professional approval, scoped user rows, review attestation, metadata-only export, and no open user browse.
- Corporate reviewer database workbench: top corporate user database surface for visible filtered users, shared records, open gaps, review attestations, visibility snapshot, access receipt, export, active filters, and no open user browse before row review.
- Corporate reviewer task command: first directory workflow for request, professional approval, scoped user rows, missing-record gaps, review attestation, metadata export, no-open-user-browse boundary, and preview-data rejection.
- Corporate row access outcome command: compact Corporate Verify answer for whether approved user rows are visible, which live Supabase/RBAC source is active, what proof is missing, and which next click saves snapshot, receipt, attestation, or metadata-only export without open user browsing.
- Corporate database next-action commander: reviewer command surface for corporate login, request-by-email, professional approval, scoped rows, missing gaps, review attestation, visibility snapshot, persisted access receipt, metadata-only export, and preview-data rejection.
- Corporate access request preflight: Verify request-form gate for active corporate role, professional email, business purpose, review window, no open user browse, and live Supabase row acceptance before access request submit.
...
- Billing choice simplification: billing panel orders pricing choice, activation workbench, launch board, pilot acceptance, and paid-launch gate before receipt/export panels, preserving Stripe-off and live Supabase ledger proof.
- Portal daily command center: first signed-in operator surface for login/logout, Professional Passport, Corporate Verify, Account, Pricing, live database proof, VPS sync, no-open-user-browse boundary, and preview-data rejection in one bounded responsive command path.
- Public login command center: first public auth surface for Professional/Corporate choice, Register/Login mode, price, first live database write, recovery, landing portal, scoped Corporate database boundary, server-save status, and credential handoff.
...
- Live data loading command: first signed-in live database command showing hosted login state, next missing Supabase row group, live row counts, preview/fixture rejection, live pilot seed, proof view, export action, and server-save status before real-row acceptance.
- Real row acceptance gate: first signed-in database blocker requiring hosted login, registration, Passport, evidence, Corporate access, consent, team, billing, review, visibility snapshot, and release ledger rows from Supabase before V1 acceptance; non-live preview rows are rejected.
- Portal server save commander: visible daily-workspace server-save proof for GitHub `main`, TrustGraph VPS target, exact `/opt/trustgraph` update command, release-stamp verification, automatic deploy secret blocker, and VFIX route protection.
- VPS freshness checkpoint: top signed-in server proof surface for GitHub source, Pages smoke, VPS release stamp, manual save command, required deploy secrets, release-stamp URL, and VFIX route protection before accepting the server as current. The release stamp is a real exported JSON asset, and the VPS updater overwrites it with the saved Git commit so HTML fallback responses are treated as stale server proof.
- Server release save path packet: GitHub source, GitHub Pages verification, TrustGraph VPS update command, hosted verification command, hosted version receipt, server HEAD match requirement, bundle smoke requirement, and VFIX isolation guard. CI verifies the release stamp asset before Pages upload and the shared freshness checker rejects HTML fallback at the release-stamp URL.
...
- Registration outcome command: visible public auth checkpoint for selected portal, login/register mode, required fields, selected price, first database write, required registration-intent completion row, landing portal, recovery, server freshness, submit, and export before portal completion.
...
- Public registration pricing gate: credentials-adjacent selected portal, register/login mode, pilot price, pricing plan id, first live database write, `registration_intents`, Stripe checkout boundary, server-save status, and live-row-only acceptance before form submission.
- Hosted auth recovery board: pre-login recovery surface for Professional or Corporate path, hosted redirect, Supabase email-rate-limit guidance, localhost link repair, resend/reset actions, export packet, and preview-data rejection before another email request is sent.
- Public auth simplification layer: visible login/register path keeps portal choice, route board, pricing, recovery, and credentials while older receipt panels remain available for source exports and CI markers instead of crowding the buyer flow.
- Public auth experience studio: top public login/register surface for Professional, Corporate, Register, Login, pricing, first database write, recovery, submit, server-save status, and preview-data rejection before dense receipts or credential fields.
- Public portal route shell: one bounded login/register surface for Professional, Corporate, pricing, first database write, landing portal, recovery, server-save status, corporate database boundary, and preview-data rejection before credentials.
- Corporate access route preview: public auth surface showing company workspace, pilot pricing ledger, request-by-email access, professional approval, approved scoped rows, no open user database browse, and live-row-only acceptance before credentials.
- Public pricing launch decision: buyer-facing pricing section now shows Professional free, Corporate `$149` pilot, selected first database write, scoped Corporate access, Stripe-off boundary, and exportable launch decision before signup.
- Public pre-signup acceptance gate: public launch checklist shows portal choice, hosted auth readiness, selected price, first live database write, Corporate scoped database boundary, live-data contract, and VPS release-stamp requirement before pilot testing.
- Public submit readiness strip: visible auth-card readiness surface shows required fields, selected first database write, completion status, recovery availability, and preview-data rejection before auth submit.
- Public portal switchboard: visible auth-card switchboard for Professional login, Corporate login, Professional registration, Corporate registration, pricing review, recovery route, first database write, and dense-receipt reduction before form submission.
- Current build server gate: public login/register and signed-in portal gate rejecting the TrustGraph VPS as current until GitHub main is green, Pages smoke passes, the VPS release stamp returns commit JSON, the served bundle contains the current TrustGraph marker, and VFIX stays unchanged.
...
- Operator viewport polish: final signed-in workspace layer caps oversized headings, stacks Passport detail panels before tablet widths, clips active workspace overflow, and keeps mobile controls readable without horizontal scrolling.
- Corporate setup simplification layer: the company-admin path keeps the launch cockpit, setup command bar, and route deck visible while duplicate helper panels remain source/export evidence instead of crowding workspace setup.
- Corporate setup triage board: first company-admin blocker surface for hosted login, corporate workspace, RBAC/admin role, team, billing ledger, scoped user database rows, direct next action, database source, export proof, and preview-data rejection.
- Corporate database simplification layer: reviewer database keeps the workbench, path strip, filters, scoped rows, access reviews, and row actions visible while duplicate queue, readiness, live-proof, and persisted-acceptance proof panels stay exportable through the workbench instead of crowding the main Corporate Verify flow.
...
- Public auth help strip: visible login/register recovery controls for verification resend, password reset, and hosted repair of localhost Supabase callback links without reopening the dense proof panels.
- Public account access path: visible pre-credential route for Professional login, Corporate login, Professional registration, Corporate registration, pricing, first database write, landing portal, recovery, server freshness, and no-preview-data acceptance.
- Public account route confirmation: first auth-card confirmation for selected portal, login/register mode, database write, landing portal, Corporate scoped-access boundary, recovery action, export proof, and preview-data rejection before submit.
- Daily SaaS route simplification: signed-in route shell is first after the header, large workspace headings are reduced, account/logout and Corporate Verify controls stay bounded, and dense proof panels sit below the primary Professional/Corporate/Admin/Pricing/Database/Server route surface.
- VPS save handoff: signed-in route shell shows green GitHub source, `save-vps` blocker, missing deploy secrets, manual `/opt/trustgraph` update command, release-stamp JSON check, and VFIX boundary before the VPS URL is accepted as current.
- Public auth route simplification: login/register card keeps account choice, Professional or Corporate switchboard, pricing/database-write confirmation, Corporate scoped-access boundary, and submit readiness before credentials while legacy proof receipts stay out of the primary buyer form path.
- Corporate reviewer queue simplification: Corporate Verify user database orders reviewer task, row-access outcome, workbench, filters, and rows before proof-heavy path receipts and persisted export packets, preserving no-open-user-browse and metadata-only export boundaries.
- Corporate review studio: top Corporate Verify user database surface for request, approval, scoped rows, gaps, review attestation, metadata-only export, preview-data rejection, and no-open-user-browse proof before filters or rows.
- Corporate database handoff command: setup-center action path connecting company workspace, RBAC/team, pricing ledger, access request, professional approval, scoped user rows, metadata export, preview-data rejection, and no-open-user-browse before deeper panels.
- Portal launch matrix: top signed-in workspace surface for Professional user, Corporate reviewer, Company Admin, pricing, account/logout, database proof, server sync, mobile stacking, and no-preview-data proof before dense panels.
- VPS saved portal command: live-server first-screen surface for server-save status, GitHub source, Professional portal, Corporate Verify, account/logout, pricing, database proof, VFIX protection, mobile stacking, and dense-dashboard deconfliction.
- Public account entry launchpad: first login/register card surface for User registration, User login, Corporate registration, Corporate login, selected pricing, first database write, recovery, submit, server status, and no-open-user-browse proof before credentials.
- Public auth start strip: top public login/register control for Professional registration, Professional login, Corporate registration, Corporate login, price, first live database write, landing portal, recovery, scoped Corporate database boundary, server status, and preview-data rejection before credentials.
...
- Evidence action queue: Passport evidence command path for metadata creation, short-lived signed preview, short-lived signed download, metadata-only manifest export, evidence packet export, raw-private-file exclusion, and preview-data rejection before the evidence table.
...
- Live database repair verifier: CI script that locks the missing-row command list, live seed action, next missing group route, proof reload route, working-data export, preview-data rejection, responsive selectors, and evidence map coverage before Pages deployment.
- Live row activation handoff: signed-in action path for hosted login, preparing live pilot rows, browser reload, seed ID reconciliation, Corporate Verify review, working-data export, and live-row-only acceptance before real database completion.
...
- V1 pilot route verifier: CI route-level script covering public website entry, Professional and Corporate registration, pricing, hosted auth recovery, Passport evidence, Corporate scoped user database, Admin exports, live database repair path, release stamp, migrations, and evidence docs before Pages deployment.
...
- V1 completion command center: first completion surface for registration, live row groups, persisted completion receipt, working-data export, VPS freshness, next missing live-row step, and preview-data rejection before the detailed completion ledger.
- Live data acceptance contract: V1 completion surface that rejects non-live preview data and requires signed-in Supabase rows, persisted completion receipt, metadata-only working-data export, and VPS release-stamp proof before V1 is treated as done.
- V1 human gate separation packet: completion surface that separates engineering-owned code/live-row/VPS freshness proof from human-owned pilot owner, Stripe, security/storage, and legal signoff before launch acceptance.
