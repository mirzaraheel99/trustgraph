...
| --- | --- | --- |
| Product foundation | `src/App.tsx`, `app/page.tsx`, `app/globals.css`, `README.md` public website, portal registration, registration intent lifecycle, contained professional/corporate auth access desk, selected portal command strip, collapsible auth operator panels, dashboard session command bar, no-rail dashboard command layout, portal readiness board, Corporate portal usability repair, V1 operating map, pricing, hosted auth guidance, and auth readiness packets | GitHub Pages workflow, `scripts/smoke-live.mjs`, `scripts/check-responsive.mjs`, `scripts/check-v1-demo-flow.mjs` |
