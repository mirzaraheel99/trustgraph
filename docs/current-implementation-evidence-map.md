...
| Professional Passport | `src/recordRepository.ts`, `src/evidenceRepository.ts`, Passport record forms, structured responsibility/skill metadata, renewal readiness packet, evidence metadata, signed preview/download controls | `supabase/migrations/001`, `012`, `017`, smoke evidence checks |
...
| Consent controls | `src/consentRepository.ts`, consent authorization and revoke UI, sensitive record controls | `supabase/migrations/025`, `026`, RLS guard |
