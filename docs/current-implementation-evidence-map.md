...
- Current build server gate: signed-in portal gate rejecting the TrustGraph VPS as current until GitHub main is green, Pages smoke passes, the VPS release stamp returns commit JSON, the served bundle contains the current TrustGraph marker, and VFIX stays unchanged.
