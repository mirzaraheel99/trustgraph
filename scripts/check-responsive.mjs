...
if (!css.includes("grid-template-columns: minmax(260px, 292px) minmax(0, 1fr)") || !css.includes("contain: inline-size") || !css.includes("max-width: 292px")) {
...
if (!css.includes(".workspace-route-strip") || !css.includes("max-width: 820px") || !css.includes(".workspace-flow-strip") || !css.includes("max-width: 900px")) {
  throw new Error("Responsive check failed: primary workspace route and portal path strips must stay compact.");
}

if (!css.includes(".workspace-command-strip") || !css.includes("grid-template-columns: minmax(190px, 0.68fr) minmax(0, 1.32fr) minmax(136px, auto)")) {
